    ...
    return (
        <div className='max-w-[1640px] m-auto px-4 py-12 '>
            ...
            <div className=' flex flex-col lg:flex-row justify-between'>
                <div><p className=' font-bold text-gray-700 '>Filter Type</p>  
                <div className=' flex justify-between flex-wrap'>
                    ...
                    <button onClick={()=>filterType('Paratha')} className=' m-1 border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white '>Paratha</button>
</div>
                    </div>
                <div><p className=' font-bold text-gray-700'>Filter Price </p>
...
{ foods.map((item,index)=>
    (
        <div key={index} className='border shadow-lg hover:scale-105 duration-300 rounded-lg'><img className='w-full h-[200px] object-cover rounded-t-lg' src={item.image} alt={item.name} />
        <div className=' flex justify-between px-2 py-4 '>
            <p className=' font-bold' >{item.name}</p>
            <p><span className=' bg-orange-500 text-white p-2 rounded'>{item.price}</span></p>
            </div>
            </div>
    )
    )}
